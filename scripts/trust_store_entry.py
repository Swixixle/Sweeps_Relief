...
This helper produces entries for SIGNING.md-format trust stores (see
docs/KEY_DISTRIBUTION.md in Sweeps_Intel). In Relief, that is the
trust_store.json at repo root, which lists Intel's public keys for
verifying Intel-signed artifacts.

It does NOT produce entries for Relief's internal signing (policy.json,
log bundles) — those use a different envelope format per Relief's legacy
signing code and do not use a trust store. Migration of Relief's internal
signing to SIGNING.md is tracked in TODO.md under 'Option C migration'.
"""
...
def main() -> None:
    p = argparse.ArgumentParser(
        description=(
            "Print a trust_store.json key entry for a public.pem (JSON to stdout). "
            "For SIGNING.md-format trust stores only; not applicable to Relief's "
            "internal flat-envelope signing."
        )
    )
